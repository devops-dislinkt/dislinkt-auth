...
def login_user():
    ...
    if not user:
        return f"not found user with username: {username}", 400
    user = User(username=user["_id"], password=user["password"], role=user["role"])

    # check password
    is_password_correct = check_password_hash(user.password, password)
    if not is_password_correct:
        return "wrong password provided", 400
...
def edit_profile_username():
    ...
    if old_username != user["_id"]:
        return "old_username not correct", 400
