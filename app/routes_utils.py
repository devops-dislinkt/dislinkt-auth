...
def check_token(f):
    ...
    def wrap(*args, **kwargs):
        ...
        try:
            ...
            found_user = mongo_api.collection("users").find_one(
                {"_id": user["username"]}
            )
            if not found_user:
                return f'not found user with username: {user["username"]}', 400
    ...
    def decorator_required_roles(f):
        ...
        def wrap(*args, **kwargs):
            try:
                ...
                if not user:
                    return f'not found user with username: {user["username"]}', 403

                user = User(
                    username=user["_id"], password=user["password"], role=user["role"]
                )
