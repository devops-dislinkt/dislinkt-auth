...
def seed_db():
    ...
    mongo_api.collection("users").insert_many(
        [
            ...
            {
                "_id": "test_djura",
                "password": generate_password_hash("test_djura123"),
                "role": "user",
            },
            {
                "_id": "test_mika",
                "password": generate_password_hash("test_mika123"),
                "role": "user",
            },
            {
                "_id": "test_mina",
                "password": generate_password_hash("test_mina123"),
                "role": "user",
            },
        ]
    )
...
class TestClassCreateUser:
    ...
    def test_create_user_success(self, client: FlaskClient):
        response = client.post(
            "/api/auth/users",
            json={"username": USER_NEW.username, "password": USER_NEW.password},
        )
        assert response.json == USER_NEW.username
        assert response.status_code == 200

    def test_create_user_without_username(self, client: FlaskClient):
        response = client.post("/api/auth/users", json={"password": USER_NEW.password})
    ...
    def test_create_user_with_existing_username(self, client: FlaskClient):
        response = client.post(
            ...
            json={
                ...
                "password": USER_INVALID_PASS.password,
            },
        )
...
class TestClassToken:
    ...
    def test_is_token_valid_with_wrong_token(self, client: FlaskClient):
        response = client.get(
            "/api/auth/is-token-valid", headers={"authorization": "Bearer 12345asdfg"}
        )
        msg = response.data.decode("UTF-8")
    ...
    def test_is_token_valid_with_good_token(self, client: FlaskClient):
        ...
        login_response = client.post(
            "/api/auth/login",
            json={"username": USER_VALID.username, "password": USER_VALID.password},
        )
        token = login_response.json
        response = client.get(
            "/api/auth/is-token-valid", headers={"authorization": f"Bearer {token}"}
        )
        msg = response.data.decode("UTF-8")
...
class TestClassLogin:
    ...
    def test_login_with_wrong_username(self, client: FlaskClient):
        response = client.post(
            "/api/auth/login",
            json={
                "username": USER_INVALID_USERNAME.username,
                "password": USER_INVALID_USERNAME.password,
            },
        )
    ...
    def test_login_with_wrong_password(self, client: FlaskClient):
        response = client.post(
            "/api/auth/login",
            json={
                "username": USER_INVALID_PASS.username,
                "password": USER_INVALID_PASS.password,
            },
        )
    ...
    def test_login_without_password(self, client: FlaskClient):
        response = client.post(
            "/api/auth/login", json={"username": USER_VALID.username}
        )
